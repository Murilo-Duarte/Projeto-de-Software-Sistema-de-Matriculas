# Projeto-de-Software-Sistema-de-Matriculas

## Documentação

* [Histórias de Usuário](documentacao/historias-de-usuario.md)
* [Código fonte do Diagrama (PlantUML)](documentacao/plantuml.txt)
![Diagrama de Casos de Uso](documentacao/diagrama.png)

## Lab01S02 - Projeto Estrutural

* [Resumo da Sprint 2](documentacao/lab01s02.md)
* [Diagrama de Classes (PlantUML)](documentacao/diagrama-classes.puml)
* [Correção do Diagrama de Casos de Uso (PlantUML)](documentacao/diagrama-casos-uso-sprint2.puml)
* Projeto Java em `src/main/java`
